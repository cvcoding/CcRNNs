...
parser.add_argument('--data', type=str, default='./data/wikitext2',
                    help='location of the data corpus (default: ./data/penn wikitext2 enwik8)')
...
parser.add_argument('--nhid_lstm', type=int, default=400,
                    help='number of hidden units per layer (default: 600)')
...
tied = args.tied
model = TCN(args.emsize, n_words, num_chans, args.nhid_lstm, dropout=dropout, emb_dropout=emb_dropout, kernel_size=k_size, tied_weights=tied)
...
def train():
    ...
    for batch_idx, i in enumerate(range(0, train_data.size(1) - 1, args.validseqlen)):
        ...
        if batch_idx % args.log_interval == 0 and batch_idx > 0:
            ...
            if cur_loss < 100:
                pplvalue = math.exp(cur_loss)
            else:
                pplvalue = cur_loss
            print('| epoch {:3d} | {:5d}/{:5d} batches | lr {:02.5f} | ms/batch {:5.5f} | '
                  'loss {:5.2f} | ppl {:8.2f}'.format(
                epoch, batch_idx, train_data.size(1) // args.validseqlen, lr,
                elapsed * 1000 / args.log_interval, cur_loss, pplvalue))
            total_loss = 0
            start_time = time.time()
...
if __name__ == "__main__":
    ...
    try:
        ...
        for epoch in range(1, args.epochs+1):
            ...
            if epoch > 5 and val_loss >= max(all_vloss[-6:]):
                lr = lr / 2.
