...
class ConvLSTMCell(nn.Module):
    ...
    def forward(self, input_tensor, cur_state):
        h_cur = cur_state.float()

        combined = torch.cat([input_tensor.float().to(self.device), h_cur.to(self.device)], dim=1)  # concatenate along channel axis
        ...
        # self.fc2.weight = torch.nn.Parameter(F.dropout(self.fc2.weight, p=self.dropout, training=self.training))

        # mask = torch.autograd.Variable(torch.ones(self.fc.weight.size(0), 1))
        # if self.fc.weight.is_cuda: mask = mask.cuda()
        # mask = torch.nn.functional.dropout(mask, p=self.dropout, training=True)
        # self.fc.weight = torch.nn.Parameter(mask.expand_as(self.fc.weight) * self.fc.weight)
        #
        # mask2 = torch.autograd.Variable(torch.ones(self.fc2.weight.size(0), 1))
        # if self.fc2.weight.is_cuda: mask2 = mask2.cuda()
        # mask2 = torch.nn.functional.dropout(mask2, p=self.dropout, training=True)
        # self.fc2.weight = torch.nn.Parameter(mask2.expand_as(self.fc2.weight) * self.fc2.weight)


        # combined_conv = self.fc(combined.squeeze()).to(self.device)

        # cc_z, cc_r = torch.split(combined_conv, self.hidden_dim, dim=1)
        cc_z = self.zw(h_cur.to(self.device)) + self.zu(input_tensor.float().to(self.device))
        cc_r = self.rw(h_cur.to(self.device)) + self.ru(input_tensor.float().to(self.device))


        z = torch.sigmoid(cc_z).to(self.device)
        r = torch.sigmoid(cc_r).to(self.device)
        rh = r * h_cur.to(self.device)
        conbined2 = torch.cat([input_tensor.float().to(self.device), rh.to(self.device)], dim=1)
        ...
        delta_u = torch.sigmoid(F.relu(F.linear(combined.squeeze(), self.w_hh))) / 1e2
        return h_next, delta_u

    def init_hidden(self, batch_size):
        # height, width = image_size
        return (torch.zeros(batch_size, self.hidden_dim, device=self.zw.weight.device),
                torch.zeros(batch_size, self.hidden_dim, device=self.zw.weight.device))
