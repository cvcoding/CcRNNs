...
class ConvLSTMCell(nn.Module):
    ...
    def reset_weigths(self):
        ...
        self.bias = Parameter(torch.tensor(1e1))
    ...
    def forward(self, input_tensor, cur_state):
        ...
        combined_conv = self.fc2(combined.squeeze()).to(self.device)
        ...
        h_next = torch.tanh(combined_conv)
    ...
    def init_hidden(self, batch_size):
        ...
        return (torch.zeros(batch_size, self.hidden_dim, device=self.fc2.weight.device),
                torch.zeros(batch_size, self.hidden_dim, device=self.fc2.weight.device))
...
class ConvLSTM(nn.Module):
    ...
    def __init__(self, input_size, num_channels, input_dim, hidden_dim, kernel_size, num_layers, dropout,
        ...
        # self.LN = nn.LayerNorm(hidden_dim, eps=0, elementwise_affine=True)
        self.LN = nn.BatchNorm1d(hidden_dim[0], eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        ...
        for t in range(seq_len):
            ...
            for layer_idx in range(self.num_layers):
                ...
                if layer_idx == 0:
                    ...
                    cur_layer_input = self.dropout(cur_layer_input)  # dropout on each latm layer
                ...
                if (pointer >= 0.99) or t == seq_len - 1:
                    ...
                    if layer_idx == self.num_layers - 1:  # ding ceng
                        ...
                        layer_idx = self.num_layers - 1  # 淇敼缁撴潫
                    # else:
                    #     if layer_idx == self.num_layers - 1:
                    #         c[layer_idx] = last_c
                    #         h[layer_idx] = last_h  # h[layer_idx-1] 鐩存帴澶嶅埗涓嬩竴灞傜殑闅愬眰杈撳嚭鍊?

                    # if layer_idx == self.num_layers - 1:
                    #     output_inner[layer_idx].append(h[layer_idx].to(self.device))
